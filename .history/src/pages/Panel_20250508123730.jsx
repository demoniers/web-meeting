...
export default function Panel() {
  ...
  const handleUpload = async () => {
    ...
    await fetch("http://localhost:3000/galeria", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: formData,
    });
  ...
  const handleDeleteMedia = async (id) => {
    const token = localStorage.getItem("token");
    await fetch(`http://localhost:3000/galeria/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  return (
    <div className="p-6">
      ...
      {/* 📢 Gestión de Noticias */}
      <h2 className="text-orange-400 text-2xl mt-6">📢 Noticias</h2>
      ...
      <button className="bg-red-500 p-2 text-black" onClick={handleCreateNews}>Publicar</button>
